...
// selectize search
$(function () {
	$('.goods-tools').submit(function (e) {
		e.preventDefault();
	});

	$('.tools-search-text').selectize({
		options: [
	...
	$('.selectize-input input').focus(function () {
		$('.tools-search-button').addClass('gray');
		if ($(window).width() < 1150) {
			$(this).closest('.goods-tools').find('.tools-dropbox').css('display', 'none');
		}
	});

	$('.selectize-input input').blur(function () {
		$('.tools-search-button').removeClass('gray');
		if ($(window).width() < 1150) {
			$(this).closest('.goods-tools').find('.tools-dropbox').css('display', 'block');
		}
	});
});